...
fn main() {
    ...
    let snps = vcf::parse_vcf(&args.vcf, args.region.clone(), args.snp_num, args.min_ratio, args.max_ratio, args.bed).unwrap();
    // info!("snps: {:?}", snps);
    // debug!("args.input: {:?}", args.input);
    let (read_snp_map, snp_ratio_dict,map_count) = bam::get_bam_dict(&args.input, &snps);
    // info!("read_snp_map: {:?}", read_snp_map);
    let haparrays = bam::process_mutation_data(&read_snp_map, &snp_ratio_dict);
    // info!("Hap save: {:?}", haparrays);
    let hap_num = hap::cal_haplotype(haparrays, &snps, args.min_proportion, args.min_count, &args.method);
    println!("Haplotype num: {}\tRecord num: {}", hap_num,map_count);
}
